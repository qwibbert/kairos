<script lang="ts">
  ...
  async function handleDndFinalize(e) {
    ...
    const new_position =
      (page - 1) * 5 +
      (e.detail.items as VineTreeItem[]).findIndex((item) => item.id == id);
  ...
  async function action_update_vine(vine: Vine) {
    await update_vine(vine.id, {
      title:
        vine_title_input_value == "" ? "Nieuwe taak" : vine_title_input_value,
    });
    vine_editing = null;
          ...
          >
            <li>
              <a
                href="."
                onclick={async () =>
                  await action_add_vine(resolved_parent_vine)}
