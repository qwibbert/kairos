...
import { add_vine, archive_vine, get_all_children, get_all_parents, get_vine, update_vine } from "./vines";
...
describe('vines db functions', () => {
    ...
    describe('add_vine function', () => {
        ...
        it('throws if empty title.', async () => {
            await expect(add_vine('', VineType.Task ,undefined, 'PARENT_1')).rejects.toThrow(expect.objectContaining({ name: 'EMPTY_TITLE' }));
        });
    });

    describe('update vine function', () => {
        it('updates a vine', async () => {
            await update_vine('PARENT_1', { title: 'PARENT', session_aim: 1, parent_id: 'CHILD_1', status: VineStatus.Active, archived: 1, public: 1, type: VineType.Course });
        ...
        it('throws if vine doesn\'t exist', async () => {
            await expect(
                update_vine('TESTvine', { title: 'PARENT', session_aim: 1, parent_id: 'CHILD_1', status: VineStatus.Active, archived: 1 })
    ...
    describe('archive_vine function', () => {
        ...
        it('throws if vine doesn\'t exist', async () => {
            await expect(archive_vine('vine')).rejects.toThrow(expect.objectContaining({ name: 'NOT_PRESENT' }));
        })
    })
})