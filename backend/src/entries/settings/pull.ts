...
export default function (e: core.RequestEvent) {
    ...
    if (!version || version != __KAIROS_VERSION__) {
        return e.json(426, ServerErrorFactory.version('settings.pull', id, { request_version: version, server_version: __KAIROS_VERSION__ }))
    }

    let documents = $app.findRecordsByFilter(
        'settings', 'user = {:user} && (updated_at > {:updated_at} || (updated_at = {:updated_at} && id != {:id}))',
        '+updated_at, +id',
        parseInt(body.batch_size, 10),
        0,
        { user: e.auth?.id, updated_at: updated_at.replace('T', ' '), id }
    );
