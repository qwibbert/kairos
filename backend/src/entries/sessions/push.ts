...
export default function (e: core.RequestEvent) {
  ...
  if (!version || version != __KAIROS_VERSION__) {
    return e.json(
      426,
      ServerErrorFactory.version("session.pull", e.requestInfo().body.id, {
  ...
  $app.runInTransaction((tx) => {
    ...
    for (const change_row of change_rows) {
      ...
      try {
        real_master_state = tx.findFirstRecordByFilter(
          sessions_collection,
          "user = {:user} && id = {:id}",
          { user: e.auth?.id, id: change_row.newDocumentState?.id },
        );
      } catch (err) {
        if (err.toString() == "GoError: sql: no rows in result set") {
          ...
          real_master_state = new Record(sessions_collection, {
            id: $security.randomStringByRegex(
      ...
      if (!new_record) {
        if (
          ...
          (real_master_state &&
             ...
             */
            new Date(real_master_state.get("updated_at"))
              .toISOString()
              .replace("T", " ") !== change_row.assumedMasterState.updated_at)
        ) {
          // we have a conflict
          ...
          event.checkpoint = {
            id: change_row.newDocumentState?.id,
        ...
        try {
          ...
          if (
            err.toString() ==
            "GoError: vine: Failed to find all relation records with the provided ids."
          ) {
            return e.json(
              424,
              HistorySyncErrorFactory.parent_rel(
                "sessions.push",
                change_row.newDocumentState?.id ?? "???",
                {
            ...
            err.toString() ==
            "GoError: vine_course: Failed to find all relation records with the provided ids."
          ) {
            return e.json(
              424,
              HistorySyncErrorFactory.course_rel(
                "sessions.push",
                change_row.newDocumentState?.id ?? "???",
                {
