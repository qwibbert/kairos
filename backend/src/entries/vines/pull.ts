...
export default function (e: core.RequestEvent) {
    ...
    if (!version || version != __KAIROS_VERSION__) {
        return e.json(426, ServerErrorFactory.version('vines.pull', id, { request_version: version, server_version: __KAIROS_VERSION__ }))
    }

    let documents = $app.findRecordsByFilter(
        'vines', 'user = {:user} && (updated_at > {:updated_at} || (updated_at = {:updated_at} && id != {:id}))',
        '+updated_at, +id',
