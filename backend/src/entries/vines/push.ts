...
export default function (e: core.RequestEvent) {
  ...
  if (!version || version != __KAIROS_VERSION__) {
    return e.json(
      426,
      ServerErrorFactory.version("vines.push", e.requestInfo().body.id, {
    ...
    for (const change_row of change_rows) {
      ...
      if (!new_record) {
        ...
        try {
          ...
          if (
            err.toString() ==
            "GoError: parent: Failed to find all relation records with the provided ids."
          ) {
            return e.json(
              424,
              VinesSyncErrorFactory.parent_rel(
                "vines.push",
            ...
            err.toString() ==
            "GoError: course: Failed to find all relation records with the provided ids."
          ) {
            return e.json(
              424,
              VinesSyncErrorFactory.course_rel(
                "vines.push",
                change_row.newDocumentState?.id ?? "???",
                {
