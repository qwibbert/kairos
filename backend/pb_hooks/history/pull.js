...
function pull_default(e) {
    ...
    $app.db().select("*").from("sessions").where($dbx.exp("user = {:user}", {
        user: (_e_auth = e.auth) === null || _e_auth === void 0 ? void 0 : _e_auth.id
    })).andWhere($dbx.or($dbx.exp("updated_at > {:updated_at}", {
        updated_at: updated_at
    }), $dbx.and($dbx.exp("updated_at = {:updated_at}", {
        updated_at: updated_at
    }), $dbx.exp("id = {:id}", {
        id: id
