...
function pull_default(e) {
    ...
    $app.db().select("*").from("settings").where($dbx.exp("user = {:user}", {
        ...
        id: id
    })))).limit(parseInt(req.query.batchSize, 10)).orderBy("updated ASC", "id ASC").all(documents);
    var new_checkpoint = documents.length === 0 ? {
        id: id,
        updated_at: updated_at
    } : {
        id: (0, import_rxdb.lastOfArray)(documents).id,
        updatedAt: (0, import_rxdb.lastOfArray)(documents).updated
    };
