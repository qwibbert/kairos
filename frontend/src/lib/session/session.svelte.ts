...
// import { add_history_entry, update_history_entry } from "../../db/history";
// import { get_settings } from "../../db/settings";
...
//     elapsed_time = $state<{ [key: string]: number }>({ [(new Date()).toDateString()]: 0 });
//     pauses: Pauses[] = [];
//     cycle = 1;
//     interval = 0;
//     time_end = 0;
...
//             const now = Date.now();
//             this.pauses.push({ timestamp: this.paused_at, duration: Math.floor((now - new Date(this.paused_at).getTime()) / 1000) });
...
//             this.elapsed_time[today] = (this.elapsed_time[today] ?? 0) + delta_t;

//             await tick_sound();

//             document.title = `${this.minutes}:${this.seconds < 10 ? '0' : ''}${this.seconds} | Kairos`;

//             // We store this in localStorage to avoid constantly syncing the elapsed time. On session restore or all other
//             // statusses that result in the session being stopped, we retrieve the value from localstorage and update the database.
//             localStorage.setItem(this.uuid, JSON.stringify(this.elapsed_time));
...
//                 await update_history_entry(this.uuid, { status: this.status, date_finished: new Date(), elapsed_time: $state.snapshot(this.elapsed_time) });
//                 await this.next();
//             }
//         }, 1000)
//     }
...
//         if (!elapsed_time) {
//             throw new SessionError({ name: 'INVALID_ELAPSED_TIME', message: 'Could not retrieve elapsed time from localStorage. Maybe the program does not have the correct storage permissions?' });
//         }

//         await update_history_entry(this.uuid, { status: this.status, paused_at: this.paused_at, elapsed_time: JSON.parse(elapsed_time) });
//     }
...
//         if (this.status == SessionStatus.Paused && this.paused_at) {
//             this.pauses.push({ timestamp: this.paused_at, duration: Math.floor((Date.now() - new Date(this.paused_at).getTime()) / 1000) });
//         }
...
//         // Retrieve the updated elapsed time from localStorage
//         const elapsed_time = localStorage.getItem(this.uuid);

//         if (!elapsed_time) {
//             throw new SessionError({ name: 'INVALID_ELAPSED_TIME', message: 'Could not retrieve elapsed time from localStorage. Maybe the program does not have the correct storage permissions?' });
//         }

//         await update_history_entry(this.uuid, { status: this.status, pauses: $state.snapshot(this.pauses), date_finished: new Date(), elapsed_time: JSON.parse(elapsed_time) });

//         // Delete elapsed time from localStorage to avoid creating a mess
...
//         if (!settings) {
//             throw new SessionError({ name: 'OTHER', message: 'Failed to retrieve settings.' });
//         }

//         const { long_break_time, short_break_time, pomo_time } = settings;
...
//     const { ui_sounds, ui_sounds_volume } = settings;
...
//     const { tick_sound, tick_sound_volume } = settings;
...
//     const { timer_sound, timer_sound_volume } = settings;
...
// export const elapsed_time_sum = (elapsed_time: { [key: string]: number }) => {
//     let sum = 0;

//     for (const value of Object.values(elapsed_time)) {
//         sum += value;
//     }
