...
describe('session', () => {
    ...
    describe('constructor function', () => {
        ...
        it('throws when invalid time is provided', () => {
            expect(() => new Session(PomoType.ShortBreak, -1)).toThrow(expect.objectContaining({ name: 'INVALID_TIME' }));
            expect(() => new Session(PomoType.ShortBreak, 0)).toThrow(expect.objectContaining({ name: 'INVALID_TIME' }));
            expect(() => new Session(PomoType.ShortBreak, 10000 * 60)).toThrow(expect.objectContaining({ name: 'INVALID_TIME' }));
        })
    });
    ...
    describe('start function', () => {
        ...
        it('throws if invalid target time', async () => {
            session.time_aim = 0;
            await expect(session.start()).rejects.toThrow(expect.objectContaining({ name: 'INVALID_TIME' }));
        });
    ...
    describe('pause function', () => {
        ...
        it('throws if session not active', async () => {
            await expect(session.pause()).rejects.toThrow(expect.objectContaining({ name: 'NOT_ACTIVE' }));
        });
    });
